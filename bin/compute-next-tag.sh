...
#
# Tags look like `v<Healthchecks version>-<release>`:
#
# - if defaults/main.yml points at a Healthchecks version that has never been
#   released, the release counter restarts at 0 (`v4.3-0`)
# - otherwise the counter is incremented (`v4.3-1`), but only if something
...
# Anchored at the start of the line, so that the variables which derive from
# this one (`healthchecks_container_image_tag` and friends) cannot match, and
# neither can the `# renovate:` annotation that sits above it.
version="$(sed -nE 's|^healthchecks_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the Healthchecks version from $defaults_path"
	exit 1
fi

# The version values already carry a leading `v` (e.g. `v4.3`),
